...
// Issues:
//  [ ] Platform: Multi-viewport: ParentViewportID not honored, and so
...
//  ImGuiPlatformIO interface. 2024-07-08: *BREAKING* Renamed
...
void
ImGui_ImplGlfw_ScrollCallback(GLFWwindow* window, double xoffset,
                              double yoffset) {
  ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
...
void
ImGui_ImplGlfw_KeyCallback(GLFWwindow* window, int keycode, int scancode,
                           int action, int mods) {
  ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
...
static bool
ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks,
...
ImGui_ImplGlfw_UpdateMouseData() {
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n++) {
    ...
    // with the _NoInputs flag (since we implement hit via our WndProc hook)
    //       On other platforms we rely on the library fallbacking to its own
    //       search when reporting a viewport with _NoInputs flag.
...
#ifdef EMSCRIPTEN_USE_EMBEDDED_GLFW3
static EM_BOOL
ImGui_ImplGlfw_OnCanvasSizeChange(int                      event_type,
...
void
ImGui_ImplGlfw_InstallEmscriptenCallbacks(GLFWwindow*,
      ...
      for (int i = 0; i < IM_ARRAYSIZE(bd->KeyOwnerWindows); i++)
        if (bd->KeyOwnerWindows[i] == vd->Window)
          ImGui_ImplGlfw_KeyCallback(
              ...
              0);  // Later params are only used for main viewport, on which
...
static void
ImGui_ImplGlfw_RenderWindow(ImGuiViewport* viewport, void*) {
  ImGui_ImplGlfw_Data*         bd = ImGui_ImplGlfw_GetBackendData();
  ImGui_ImplGlfw_ViewportData* vd =
      (ImGui_ImplGlfw_ViewportData*)viewport->PlatformUserData;
...
static void
ImGui_ImplGlfw_SwapBuffers(ImGuiViewport* viewport, void*) {
  ImGui_ImplGlfw_Data*         bd = ImGui_ImplGlfw_GetBackendData();
  ImGui_ImplGlfw_ViewportData* vd =
      (ImGui_ImplGlfw_ViewportData*)viewport->PlatformUserData;
  if (bd->ClientApi == GlfwClientApi_OpenGL) {
...
static int
ImGui_ImplGlfw_CreateVkSurface(ImGuiViewport* viewport, ImU64 vk_instance,
...
ImGui_ImplGlfw_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    ...
    case WM_XBUTTONUP:
      ...
      // We have submitted https://github.com/glfw/glfw/pull/1568 to allow GLFW
      // to support "transparent inputs". In the meanwhile we implement custom
      // per-platform workarounds here (FIXME-VIEWPORT: Implement same
